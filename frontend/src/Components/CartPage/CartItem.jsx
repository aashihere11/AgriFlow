...
function CartItem({ item, Checked, onCheck, onQtyChange, onDelete }) {
  ...
  return (
    <div className={`flex items-center gap-3 py-3 border-b border-gray-100 last:border-none transition-opacity  ${!Checked ? "opacity-40" : ""}`}>
      ...
      {/* IMAGE */}
      <div className="w-10 h-10 md:w-20 md:h-20 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[10px] md:text-base font-medium text-gray-800 m-0">{item.name}</p>
        <p className="text-[10px] md:text-base text-gray-400 mt-0.5 m-0">₹{item.price} / {item.unit}</p>
